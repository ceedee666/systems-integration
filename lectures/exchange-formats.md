...
1. Does the XML document conform to the schema? Why or why not? If not, what changes would be needed?
2. How would the validation fail if the `country` field had more than 2 characters?
3. What happens if the `email` field is missing, given that it's a mandatory field?
